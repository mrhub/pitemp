...
<?php
   ...
   if (substr($dropvalue, 0, 1) == "6") {
    $sql =<<<EOF
      select * from temps where timestamp > datetime('now', 'localtime', '-6 hours'); 
EOF;
   } else if (substr($dropvalue, 0, 1) == "1"){
$sql =<<<EOF
      select * from temps where timestamp > datetime('now', 'localtime', '-1 hours'); 
EOF;
   } else if (substr($dropvalue, 0, 2) == "24") {
$sql =<<<EOF
      select * from temps where timestamp > datetime('now', 'localtime', '-24 hours'); 
EOF;
   } else{

       $sql = "select * from temps where timestamp > datetime('" . $dtp_input1 . "', 'localtime')";
       $sql = $sql . " and timestamp < datetime('" . $dtp_input2 . "', 'localtime')";
       echo $sql . "</br>";
   }
